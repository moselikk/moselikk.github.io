...
export function renderPostPage(post: PostDetailOptions): string {
  const safeDate = isNaN(post.date.getTime()) ? new Date() : post.date;
  ...
  <header class="post-header">
    ...
    <p class="post-meta">
      ...
      <time datetime="${isoDate}" itemprop="datePublished">${formattedDate}</time>${post.author ? ` • <span itemprop="author" itemscope itemtype="http://schema.org/Person"><span itemprop="name">${post.author}</span></span>` : ''}</span>
    ...
    document.addEventListener("DOMContentLoaded", function() {
      if (typeof Gitalk !== "undefined") {
        ...
        var gitalk = new Gitalk({
          ...
          admin: ${JSON.stringify(siteConfig.gitalk.admin)},
          id: title,
  ...
  return renderLayout({
    ...
    canonicalUrl: `${siteConfig.url}${post.url}`,
    type: 'article',
